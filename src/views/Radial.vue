<template>
  <v-app style="background-color: #121212">
    <v-container class="d-flex flex-row justify-center">
      ...
      <button style="background-color: green;max-height: 20px" @click="openMenu">{{ menuState }}</button>
      <!--    <button @click="closeMenu">Close Menu</button>-->
      <RadialMenu ref="radialMenu" @clicked="menuClicked" :menu-items="menuItems" :size="400"></RadialMenu>
    </v-container>
  </v-app>
</template>
...
export default {
  ...
  methods: {
    menuClicked: function (menuItem) {
      router.push({name: menuItem.title})
    },
    openMenu: function () {
      if (this.menuState === 'Open Menu') {
        this.$refs.radialMenu.open();
        this.menuState = 'Close Menu'
      } else {
        this.$refs.radialMenu.close();
        this.menuState = 'Open Menu'
      }
    },
    closeMenu: function () {
      this.$refs.radialMenu.close();
